function [x_out, output] = genHyBR_adjoint(b, Q, R, options)
%
% [x_out, output] = genHyBR_adjoint(b, Q, R, options)
%
...
% Note the forward and adjoint models (A and A') will be called using the functions
%	'afun' and 'afunadj'. 
%
% Inputs:
%     b : rhs vector
...
%	 Unknownmean -- Set to 'on' if you are solving the unknown mean case.
%
...
% S. Miller, modified in 2023 for GEOS-Chem problem

%% Initialization
defaultopt = struct('InSolv','tikhonov','RegPar','wgcv','nLevel', 'est',...
  'Omega', 'adapt', 'Iter', [] , 'Reorth', 'on', 'x_true', 'off', 'BegReg', 2,...
  'Vx' , [], 'FlatTol', 10^-6, 'MinTol', 10, 'ResTol', [10^-6, 10^-6], 'Unknownmean','on');
...
% Initialize to handle adjoint case (e.g., GEOS-Chem)
[U, B, V, QV] = feval(GKhandle, Q, R, U, B, V, QV, options);
  ...
  if i >= 2 %Begin GK iterations
    ...
    switch insolve
      case {'tsvd', 'tikhonov'}
        ...
        [f, alpha] = feval(solverhandle, Ub, diag(Sb), Vb, vector, options, B(:,1:end-1), beta, QV, m);
...
%         f = B \ vector;
	...
	% Calculate the residual norm and check for convergence.
	% Originally, the code calculated the residuals by running the estimate through the
	% forward model. The code has been revised here to avoid a call to the forward/adjoint
	% models based on equations in Chung and Saibaba 2017. 
	% The variable "z" in Chung and Saibaba, "y" in Cho et al., and "f" in this script
	% x = QVy where x = inv(Q)*(s - mu)
	% B in this script: I assume B in the manuscript and code are the same.
	% lower case delta in this script: It's a coefficient in Golub Kahan and is in B
	% e in this script: delta * e = vector in this script
    %%     x = Q*(V*f);
    x = QV*f;
    % r = b(:) - A*x(:);
    Bk = B(:,1:end-1);
...
%    end
    I     = eye(i);
    Bline = [(Bk'*Bk); B(i+1,i+1).*B(i+1,i).*I(:,i)'];

    ATRr  = V * (Bline * f - B(1,1).*vector);
    ATRr  = ATRr(1:sizen(Q),:);
    ...
    if normr <= restol(1)*beta+restol(2)*norma*normx || normar/(norma*normr) <= restol(2) && terminate
        disp('Exiting based on residual convergence criteria');
   	if notrue %Set all the output parameters and return
        if outputparams
          output.U = U;
          output.V = V;
